...
export const RESOURCE_TYPES = {
  ...
  NUKA_COLA: {
    key: "NukaColaQuantum",
    display: "Nuka Cola Quantum",
    max: GAME_LIMITS.NUKA_COLA_MAX,
  },
} as const;

// Validation functions
export const validateValue = {
  special: (value: number): number =>
    Math.max(GAME_LIMITS.SPECIAL_MIN, Math.min(GAME_LIMITS.SPECIAL_MAX, value)),
  happiness: (value: number): number =>
    Math.max(GAME_LIMITS.HAPPINESS_MIN, Math.min(GAME_LIMITS.HAPPINESS_MAX, value)),
  health: (value: number): number =>
    Math.max(GAME_LIMITS.HEALTH_MIN, Math.min(GAME_LIMITS.HEALTH_MAX, value)),
  level: (value: number): number =>
    Math.max(GAME_LIMITS.LEVEL_MIN, Math.min(GAME_LIMITS.LEVEL_MAX, value)),
  radiation: (value: number): number =>
    Math.max(GAME_LIMITS.RADIATION_MIN, Math.min(GAME_LIMITS.RADIATION_MAX, value)),
  ...
  petCarriers: (value: number): number =>
    Math.max(0, Math.min(GAME_LIMITS.PET_CARRIERS_MAX, value)),
  starterPacks: (value: number): number =>
    Math.max(0, Math.min(GAME_LIMITS.STARTER_PACKS_MAX, value)),
} as const;
